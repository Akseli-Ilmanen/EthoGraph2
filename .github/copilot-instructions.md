...
align = eto.discover_trial_sources("1", ds)  # timeseries_source.discover_trial_sources()
...
**Discovery function:**
```python
alignment = eto.discover_trial_sources(
    trial_id, ds,
    video_offset=0.0,
    audio_path=..., audio_offset=0.2,       # scalar offset
    ephys_path=..., ephys_timestamps=...,    # or explicit timestamps
    spike_times=..., label_intervals=...,
)
...
#   Events:
#     spikes                TimeRange(0.100s .. 29.900s)
#     labels                TimeRange(1.500s .. 28.300s)
```

Scans dataset for xarray variables with time coordinates, optionally adds file-based audio/ephys sources, spike events, audio changepoints, and label intervals.
