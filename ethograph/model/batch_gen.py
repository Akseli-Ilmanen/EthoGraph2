...
class BatchGenerator(object):
    ...
    def read_data(self, vid_list_file, shuffle=True):
    ...
    def next_batch(self, batch_size, if_warp=False): # if_warp=True is a strong data augmentation. See grid_sampler.py for details.
        ...
        # batch_input[0].shape =(2048, 4718)
        length_of_sequences = list(map(len, batch_target))
        #batch_input_tensor([1, 2048, 4718])
        ...
        # batch_input_tensor=torch.Size([1, 2048, 4718])
        # batch_target_tensor=torch.Size([1, 4718])
        # mask=torch.Size([1, 19, 4718])
